...
class SMDP:
    ...
    def __init__(self, nr_arrivals, reporter=None, crn=None):
        """
    ...
    def step(self, action):
        ...
        sum_of_rates = self.arrivals_coming() * 0.5 + 1/1.8 * len(self.processing_r1) + 1/10 * len(self.processing_r2)
        if sum_of_rates == 0:
            return self.observation(), 0, self.is_done(), False, None
        else:
            p_arrival = (self.arrivals_coming() * 0.5) / sum_of_rates
            p_r1a_completion = (1/1.8 * len(self.processing_r1)) / sum_of_rates
            p_r2a_completion = (1/10 * len(self.processing_r2)) / sum_of_rates
            # now we draw from these distributions
            time = self.crn.generate_exponential(sum_of_rates)
            self.total_time += time
            evolution = self.crn.choice(['arrival', 'r1a', 'r2a'], weights=[p_arrival, p_r1a_completion, p_r2a_completion])
