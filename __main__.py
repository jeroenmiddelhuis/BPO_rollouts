...
def evaluate_single_policy(pl, config_type, episode_length=10, nr_rollouts=100, results_dir=None, env_type='smdp'):
    ...
    with open(results_file_path, 'a') as results_file:
        ...
        if pl_name in ['pi', 'vi']:
            rewards = rollouts.evaluate_policy(env, pl.policy, nr_rollouts, ppo=False)
        else:
...
def main():
    ...
    # # env_type = 'mdp'
    # for config_type in ['high_utilization', 'down_stream', 'low_utilization', 'parallel', 'single_activity', 'slow_server']: # ['n_system', 'high_utilization']:
    ...
    #             elif policy == 'vi':
    #                 filename = f"./models/vi/{config_type}_0.99.npy"
