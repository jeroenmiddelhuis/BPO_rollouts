...
class ValueIteration:
    ...
    def set_state_from_observation(self, env, observation):
        ...
        for state_space_label in state_space_labels:
            ...
            if self.config_type == 'single_activity' and state_space_label.startswith('is_processing'):
            ...
            if self.config_type == 'n_system' and state_space_label == 'is_processing_r1':
                ...
                if observation[state_space_labels.index('is_processing_r1')] == 0: # == 0 which means the resource is assigned
    ...
    def process_observation(self, observation, action):
        if action == 'r1a':
            observation[self.env.state_space.index('is_processing_r1')] = 0
            ...
            observation[self.env.state_space.index('is_processing_r2')] = 0
            if self.config_type != 'single_activity':
    ...
    def evolve_observation(self, observation, evolution):
        ...
        if evolution == 'return_to_state':
            ...
            observation[self.env.state_space.index('is_processing_r1')] = 1 # Resource becomes available
            if self.config_type not in ['n_system', 'single_activity']:
            ...
            observation[self.env.state_space.index('is_processing_r1')] = 1
            if self.config_type not in ['n_system', 'single_activity']:
                observation[self.env.state_space.index('assigned_r1b')] = 0
            return observation
        elif evolution == 'r2a':
            observation[self.env.state_space.index('is_processing_r2')] = 1 # Resource becomes available
            if self.config_type != 'single_activity':
            ...
            observation[self.env.state_space.index('is_processing_r2')] = 1
            if self.config_type != 'single_activity':
    ...
    def value_iteration(self, max_iterations=0):
        ...
        state = []
        while not done:
            print(f'Iteration {iteration} with delta {delta}')
            delta = 0
            for s in tqdm(self.state_space, total=len(self.state_space)):
...
def main():
    ...
    for config in ['single_activity']:
        ...
        np.save(f'{directory}/{config}_{gamma}.npy', policy)
